...
function rectangle(x=0, y=0, width=20, height=10, color = [0]) {
  start();
  
  window.defaultCanvas0.rect(x, y, width, height);
  if(color) {
...
function square(x=0, y=0, size=20, color = [0]) {
  start();
  window.defaultCanvas0.rect(x, y, size, size);
  if(color) {
    fill(color[0], color[1], color[2]);
  }
  end();
...
function circle(x=0, y=0, r=50, color) {
  start();
  window.defaultCanvas0.arc(x, y, 50, 0, 2 * Math.PI);
  if(color) {
    fill(color[0], color[1], color[2]);
  }
  end();
...
function line(from=[0, 0], to=[0, 10], size=1, color = [0]) {
  start();
