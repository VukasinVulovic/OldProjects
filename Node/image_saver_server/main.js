...
document.querySelector('.save_button').onclick = saveImage;
let canvas, cam, data, socket = new WebSocket('ws://localhost:69');

function setup() {
  cam = capture(CAMERA);
  canvas = createCanvas(400, 400, 0, 0, [20]);
...
function saveImage() {
  if(socket.OPEN == 1) {
    ...
    socket.send(`{"name": "${name}", "label": "${label}", "image": "${data}"}`);
  }
}
