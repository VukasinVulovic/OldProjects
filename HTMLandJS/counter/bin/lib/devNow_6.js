...
let KEY = '',
	ASCII = 0;
let WHEEL = 0;
let CANVAS = 'No canvas on the page.', 
...
function rectangleEl(x = 0, y = 0, width = 20, height = 10, angle = 0, color = [255]) {
	start();
	window.defaultCanvas0.translate(x + width / 2, y + height / 2);
	window.defaultCanvas0.rotate(angle * Math.PI / 180);
...
function circleEl(x = 0, y = 0, r = 50, angle = 0, color = [255]) {
	start();
...
function imageEl(x = 0, y = 0, angle = 0, height = 100, width = 100, src = 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/64px-JavaScript-logo.png') {
	let img = Element('img');
	img.src = src;
...
function textEl(x = 0, y = 0, size = 30, text = 'Hell0, World!', font = 'Arial') {
	CANVAS.font = `${size}px ${font}`;
	CANVAS.fillText(text, x, y+(size/2));
}
...
function urlParamaters(url='www.google.com?q=How') {
	...
	return JSON.parse(data.slice(0, data.length-1) + '}');
}
...
function notification(title='---title---', body='---body---', icon='https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/64px-JavaScript-logo.png', event=noFunction) {
  Notification.requestPermission().then((allowed) => {
    allowed = allowed == 'granted';
    if(allowed) {
      if(window.developer)console.log('%c Notifications are allowed.', 'color: rgb(0, 200, 0);');
      let element = new Notification(title, {
        body: body,
        icon: icon
      });
      element.addEventListener('click', event);
    } else {
      if(window.developer)console.log('%c Notifications are not allowed.', 'color: rgb(255, 0, 0);');
    }
  });
}
