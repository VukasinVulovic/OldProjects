...
function textEl(text='Hello, World!', x=0, y=0, size=30, color=[255], font='Ariel') {
  window.defaultCanvas0.font = `${size}px ${font}`;
  window.defaultCanvas0.textBaseline = 'top';
  fill(color[0], color[1], color[2]);
  window.defaultCanvas0.fillText(text, x, y);
}
...
function READ() {
 ...
 function random(min=0, max=255) {
   if(!Array.isArray(min)) {
     ...
     this.value = min[Math.floor(Math.random() * (min.length - 0) + 0)];
   }
   return this.value;
}
