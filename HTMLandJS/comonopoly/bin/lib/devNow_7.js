...
document.addEventListener('keyup', e => { KEY = '', ASCII = 0; });
...
function refresh() { CANVAS.clearRect(0, 0, WIDTH, HEIGHT); }
...
function addSlider(id=false, x=0, y=0, min=0, max=255, width=100, startValue=0) {
	let obj;
	if(!grab(id) || !id) obj = Element('input', id);
	else obj = grab(id);
	obj.id = id,
	obj.type = 'range',
	obj.min = 0,
	obj.max = 100,
	obj.value = map(startValue, min, max, 0, 100),
	obj.style.width = `${width}px`,
	obj.style.position = 'absolute',
	obj.style.left = `${x}px`;
